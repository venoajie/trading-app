...
def parse_error_message(
    ...
    formatted_error = orjson.dumps(
        error_info, option=orjson.OPT_INDENT_2 | orjson.OPT_SORT_KEYS
    ).decode("utf-8")
...
async def parse_error_message_with_redis(
    ...
    try:
        ...
        error_details = parse_error_message(error, context, additional_info)
        ...
        result["params"].update({"channel": channel, "data": pub_message})

        # Publish to Redis
        await client_redis.publish(channel, orjson.dumps(result))

    except Exception as inner_error:
        # Fallback to basic logging if Redis fails
        log.critical("FAILED TO REPORT ERROR TO REDIS")
        parse_error_message(
            inner_error, "Error handling failure", {"original_error": str(error)}
        )
