...
async def process_message_batch(
    client_redis: aioredis.Redis, messages: List, processing_state: Dict[str, Any]
) -> None:
    """
    ...
    pending = await client_redis.xpending_range(
        "stream:market_data", "dispatcher_group", min_idle_time=30000  # 30 seconds
    )
    if pending:
    ...
    for message_id, message_data in messages:
        try:
            ...
            current_server_time = (
                timestamp + server_time if server_time == 0 else timestamp
            )
...
async def stream_consumer(
    client_redis: aioredis.Redis, processing_state: Dict[str, Any]
) -> None:
    """
    ...
    await client_redis.ensure_consumer_group(
        stream="stream:market_data", group_name="dispatcher_group"
    )
...
async def caching_distributing_data(
    ...
    consumer_task = asyncio.create_task(stream_consumer(client_redis, processing_state))
...
async def handle_incremental_ticker(
    ...
    pub_message.update(
        {"instrument_name": instrument_name_future, "currency_upper": currency.upper()}
    )
...
async def handle_chart_trades(
    chart_channel: str, message_channel: str, pub_message: Dict, result_template: Dict
) -> None:
        ...
        for o in subaccounts_details_result:
            if "positions" in o:
                ...
                caching.positions_updating_cached(
                    positions_cached, positions[0], "rest"
                )
                break
